<%@ Control Language="C#" AutoEventWireup="true" CodeFile="NavControl.ascx.cs" Inherits="NavControl" %>
<%@ Import Namespace="RICH.Common" %>
<ul class="nav navbar-nav">
    <li class="active"><a id="linkDefaultIndex" runat="server">Ê×Ò³</a></li>
    <asp:Repeater ID="NavList" runat="server">
        <ItemTemplate>
            <li><a href="<%# "{0}/{1}".FormatInvariantCulture(DataBinder.Eval(Container.DataItem, "PageFilePath"), DataBinder.Eval(Container.DataItem, "PageFileName"))%>"><%# DataBinder.Eval(Container.DataItem, "PurviewName")%></a></li>
        </ItemTemplate>
    </asp:Repeater>
    <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown">Dropdown
        <b class="caret"></b></a>
        <ul class="dropdown-menu">
            <li><a href="#">Action</a></li>
            <li><a href="#">Another action</a></li>
            <li><a href="#">Something else here</a></li>
            <li class="divider"></li>
            <li class="dropdown-header">Nav header</li>
            <li><a href="#">Separated link</a></li>
            <li><a href="#">One more separated link</a></li>
        </ul>
    </li>
</ul>
