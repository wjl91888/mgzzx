...
copy T_PM_PurviewInfoScriptProc.sql ..\..\..\Database\Scripts\Post-Deployment\Proc\
copy T_PM_PurviewInfoScriptPurview.sql ..\..\..\Database\Scripts\Post-Deployment\Purview\
copy T_PM_PurviewInfoScriptUpdateField.sql ..\..\..\Database\Scripts\Post-Deployment\UpdateField\
@ECHO 开始安装T_PM_PurviewInfo数据库脚本
echo Begin > log.log
for /f "delims=" %%a in ('dir T_PM_PurviewInfoScriptProc.sql /s /b') do (sqlcmd -d DB_MGZZX -i %%a >> log.log)
echo End >> log.log
